...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (!token || !jwt.verify(token, process.env.JWT_SECRET!)) {
            return res.json({ success: false, type: "warning", message: "Vous devez être connecter pour ajouter un produit a votre panier." }, {
                status: 401
            })
        }
        ...
        try {
            ...
            return res.json({ success: true, type: "success", message: "Produit ajouté au panier." }, {
                status: 200
            })
        }
        catch (error) {
